...
KA = 10         # Сколько секунд после ответа модель будет в памяти                     |   По стандарту 10 секунд, но если вам нужна оперативная память между ответами модели, то ставьте на 0, а при фокусированном общении с ИИ - наоборот ставьте больше, чтобы каждый раз снова не загружать ИИ в память
...
def web_search(search=str, link=bool): # Поиск в интернет       # ПЕРЕДЕЛАТЬ
    try:
        if not link:
            search.replace(' ', '+')
            search.replace('\n', '+')
            response = requests.get(f'https://html.duckduckgo.com/html/?q={search}', headers={'User-Agent': 'Mozilla/5.0'})
...
tools = [
    ...
    {   "type": "function", # Перепиши это ради боба, я уже не могу [ВОТ НЕПРУХА]
        "function": {       # TODO: умереть от суицида
            "name": "web_search",
